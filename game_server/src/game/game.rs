...
fn game_loop(
    ...
    let players_controller = Arc::new(RwLock::new(PlayersControl::new(game_log_tx.clone())));
    ...
    while *running.lock().unwrap() {
            ...
            if let Ok(msg) = server_cmds_rx.recv() {
                match msg.clone() {
                    ...
                    Cmds::Player(player_cmd) => {
                        let mut _players_controller = players_controller_lock.write().unwrap();
        ...
        // Updates
        let players_controller_lock = Arc::clone(&players_controller);
        let _players_controller= players_controller_lock.write().unwrap();
